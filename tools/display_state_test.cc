...
int main() {
    ...
    assert(model.CanUsePartial() && !model.ShouldRequestFullClean());
    assert(model.state().partial_refresh_count == 0 && !model.state().has_dirty_region);
    model.OnFull4BppSuccess(); assert(!model.CanUsePartial() && !model.state().has_dirty_region);
